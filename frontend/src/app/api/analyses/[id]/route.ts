...
import { z } from "zod";
import { AnalysisStatusResponseSchema } from "@/contracts/analysis.contract";
...
export async function GET(
  ...
  try {
    ...
    // 3. Forward request to FastAPI backend status endpoint
    let response: Response;
    try {
      response = await fetch(`${backendApiUrl}/api/v1/analysis/${id}`, {
        method: "GET",
        headers: {
      ...
      console.error("BFF analysis status query network failure:", networkError);
      return NextResponse.json(
    ...
    if (statusCode >= 500) {
      console.error(`BFF received ${statusCode} from backend during analysis status check.`);
      return NextResponse.json(
    ...
    const parseResult = AnalysisStatusResponseSchema.safeParse(jsonBody);
    ...
    console.error("BFF unhandled error in analysis status route handler:", err);
    return NextResponse.json(
